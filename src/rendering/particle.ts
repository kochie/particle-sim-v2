...
import { KinematicMethod } from '../emun/KinematicMethod'
...
export class Particle {
	...
	public calcKinematics(
		dt: number,
		method: KinematicMethod = KinematicMethod.EULER,
	): void {
		switch (method) {
			...
			case KinematicMethod.RK4: {
				...
				const kv2 = a
					.clone()
					.multiply(r.clone().add(kr1.clone().multiplyScalar(dt / 2)))
				const kr2 = v
					.clone()
					.multiply(kv1)
					.multiplyScalar(dt / 2)
				...
				const kv3 = a
					.clone()
					.multiply(r.clone().add(kr2.clone().multiplyScalar(dt / 2)))
				const kr3 = v
					.clone()
					.multiply(kv2)
					.multiplyScalar(dt / 2)
				...
				const kv4 = a
					.clone()
					.multiply(r.clone().add(kr3.clone().multiplyScalar(dt)))
				const kr4 = v
					.clone()
					.multiply(kv3)
					.multiplyScalar(dt)
				...
				const hv = kv1
					.add(kv2.multiplyScalar(2))
					.add(kv3.multiplyScalar(2))
					.add(kv4)
				const hp = kr1
					.add(kr2.multiplyScalar(2))
					.add(kr3.multiplyScalar(2))
					.add(kr4)
